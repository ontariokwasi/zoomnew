...
function sendsms($id, $job_id, $user_sess, $msisdn, $sender,$sms_count,$network,$msgs,$submitdt,$created_by) {
	...
	$dlr = "http://168.144.175.52/nalosms/sendsmsdlr.php?source=%P&destination=%p&SVC=%n&msgID=%F&msg=%a&msglen=%L&timestamp=%t&status=%d&smsc=%i&smsid=%I&dlrv=%d&MDS=%D&DLRS=%A&osms=%b&mid=" . $id;
				$dlrEncode = urlencode($dlr);

				$prefix = substr($msisdn, 0,5);
				if($prefix == '23324'||$prefix == '23354'||$prefix == '23355'){
					if(checkPorted($msisdn) == 0){

						$url = "http://168.144.175.52:13013/cgi-bin/sendsms?username=guiuser&password=O14ns0&to=" . $msisdn . "&from=" . $sender . "&text=" . $msgs . "&dlr-mask=31&dlr-url=" . $dlrEncode;
					}
					else{

						$url = "http://78.46.254.50:13013/cgi-bin/sendsms?username=guiuser&password=O14ns0&to=" . $msisdn . "&from=" . $sender . "&text=" . $msgs . "&dlr-mask=31&dlr-url=" . $dlrEncode;
